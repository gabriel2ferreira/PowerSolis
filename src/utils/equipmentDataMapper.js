...
export const mapEquipmentToDisplay = (equipment) => {
  ...
  logger.info('equipmentDataMapper', `Raw API Config for Mapping (Eq ID: ${equipment.id})`, {
    temperatura_hotspot_inferida: apiConfigRaw.temperatura_hotspot_inferida,
    ponto_quente_externo: apiConfigRaw.ponto_quente_externo
  });
  ...
  const mappedData = {
    ...
    apiConfig: {
      horasOperacao: apiConfigRaw.horas_operacao != null ? Number(apiConfigRaw.horas_operacao) : 0,
      pontoQuenteExterno: apiConfigRaw.ponto_quente_externo != null ? Number(apiConfigRaw.ponto_quente_externo) : null,
      temperaturaAmbiente: apiConfigRaw.temperatura_ambiente != null ? Number(apiConfigRaw.temperatura_ambiente) : 25,
      vidaUtilIsolamentoH: apiConfigRaw.vida_util_isolamento_h != null ? Number(apiConfigRaw.vida_util_isolamento_h) : 219150,
      estrategiaEnvelhecimento: apiConfigRaw.estrategia_envelhecimento || 'N/A',
      tangentePerdas: apiConfigRaw.tangente_perdas != null ? Number(apiConfigRaw.tangente_perdas) : 0,
      correntePrimario: apiConfigRaw.corrente_primario != null ? Number(apiConfigRaw.corrente_primario) : 0,
      tempRef: apiConfigRaw.temp_ref != null ? Number(apiConfigRaw.temp_ref) : 85,
      vidaRefAnos: apiConfigRaw.vida_ref_anos != null ? Number(apiConfigRaw.vida_ref_anos) : 25,
      p: apiConfigRaw.p != null ? Number(apiConfigRaw.p) : 8,
      temperaturaHotspotInferida: apiConfigRaw.temperatura_hotspot_inferida != null ? Number(apiConfigRaw.temperatura_hotspot_inferida) : null
    },
    raw: equipment
  };

  logger.info('equipmentDataMapper', `Mapped API Config values (Eq ID: ${equipment.id})`, {
