...
import { Activity, AlertTriangle, Server, Filter } from 'lucide-react';
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      for (const item of eqData) {
        ...
        checkAndCreateAlert(item.id, healthData.status);

        const activeAlerts = alertsData.filter(a => a.equipment_id === item.id).length;
        if (activeAlerts > 0) withAlertsCount++;

        let effectiveStatus = healthData.status;
        if (effectiveStatus === 'Crítico' || item.status === 'Crítico') {
          criticalCount++;
          effectiveStatus = 'Crítico';
        }
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredEquipments.map((eq) => (
              <div key={eq.id} onClick={() => navigate(`/equipment/${eq.id}`)} className="flex flex-col justify-between p-4 border rounded-lg hover:shadow-md cursor-pointer bg-card transition-shadow">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex flex-col gap-1">
                    <h3 className="font-bold text-base truncate pr-2">{eq.name}</h3>
                    <p className="text-xs text-muted-foreground">{eq.location_name || eq.city} • {eq.type_name}</p>
                  </div>
                  <Badge style={{ backgroundColor: eq.healthData.color }} className="text-white shrink-0">{eq.healthData.status}</Badge>
                </div>
                
                <div className="flex flex-col gap-3">
                  <div>
                    <div className="flex justify-between text-xs font-medium text-muted-foreground mb-1">
                      <span>Saúde Atual</span><span style={{ color: eq.healthData.color }}>{eq.healthData.percentage.toFixed(1)}%</span>
                    </div>
                    <div className="h-2 w-full bg-secondary rounded-full overflow-hidden flex">
                      <div className="h-full" style={{ width: `${eq.healthData.percentage}%`, backgroundColor: eq.healthData.color }} />
                    </div>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="bg-muted/50 p-2 rounded flex flex-col">
                      <span className="text-muted-foreground">Vida Reman.</span>
                      <span className="font-medium">{eq.healthData.vidaRemanescenteAnos.toFixed(1)} anos</span>
                    </div>
                    <div className="bg-muted/50 p-2 rounded flex flex-col">
                      <span className="text-muted-foreground">Fat. Aceleração</span>
                      <span className={`font-medium ${eq.healthData.fatorAceleracao > 1 ? 'text-red-500' : 'text-green-500'}`}>{eq.healthData.fatorAceleracao.toFixed(2)}x</span>
                    </div>
                  </div>
                </div>
              </div>
