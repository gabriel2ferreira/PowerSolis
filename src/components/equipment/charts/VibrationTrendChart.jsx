...
const VibrationTrendChart = ({ equipmentId }) => {
    ...
    const fetchData = async () => {
      ...
      try {
        const { data: history, error } = await supabase
          ...
          .select('report_date, vibration')
          .eq('equipment_id', equipmentId)
          .not('vibration', 'is', null)
  ...
  if (data.length === 0) {
    return (
      <Card>
        <CardHeader><CardTitle className="text-sm font-medium flex items-center gap-2"><ActivitySquare className="w-4 h-4"/> Vibração</CardTitle></CardHeader>
      ...
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center">
          <CardTitle className="text-sm font-medium flex items-center gap-2">
            <ActivitySquare className="w-4 h-4 text-purple-500" /> Vibração (mm/s)
          </CardTitle>
        ...
        <div className="h-[250px] w-full mt-4">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
              ...
              <Tooltip formatter={(value) => [`${value} mm/s`, 'Vibração']} />
