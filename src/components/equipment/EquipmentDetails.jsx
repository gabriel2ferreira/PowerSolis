...
import { Loader2, Activity, Settings, Info, History, ArrowLeft } from 'lucide-react';
...
const EquipmentDetails = ({ equipmentId, onBack }) => {
  ...
  const statusInfo = useMemo(() => {
    if (!equipment) return { status: 'Desconhecido', color: '#94a3b8', healthPercentage: 0 };
    const eqForCalc = {
      ...equipment.rawPayload,
      health: equipment.health,
      api_config: equipment.apiConfig
    };
    return getEquipmentStatus(eqForCalc);
  }, [equipment]);
            ...
            <div className="flex flex-col md:flex-row gap-8 items-center justify-center mb-6">
              <div className="flex flex-col items-center">
                <span className="text-6xl font-black" style={{ color: statusInfo.color }}>
                  {statusInfo.healthPercentage.toFixed(1)}%
                </span>
                <Badge 
                  variant="outline" 
              ...
              <div className="grid grid-cols-2 gap-4 w-full max-w-sm">
                <div className="bg-secondary/50 p-3 rounded-lg flex flex-col">
                  ...
                  <span className="text-lg font-bold">{equipment.health?.vidaRemanescenteAnos?.toFixed(1) || 0} anos</span>
                </div>
                <div className="bg-secondary/50 p-3 rounded-lg flex flex-col">
                  <span className="text-xs text-muted-foreground font-medium uppercase tracking-wider mb-1">Fat. Aceleração</span>
                  <span className={`text-lg font-bold ${(equipment.health?.fatorAceleracao || 1) > 1 ? 'text-destructive' : 'text-primary'}`}>
                    {(equipment.health?.fatorAceleracao || 1.0).toFixed(2)}x
                  </span>
                </div>
                <div className="bg-secondary/50 p-3 rounded-lg flex flex-col col-span-2">
                  ...
                  <span className="text-sm font-bold capitalize">{equipment.health?.estrategiaUsada || 'Desconhecido'}</span>
                </div>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="pt-6">
            <div className="grid grid-cols-2 gap-y-4 gap-x-6">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-sm font-semibold">{equipment.status}</span>
              </div>
              <div className="flex flex-col">
        ...
        <Card className="shadow-sm">
          ...
          <CardContent className="pt-6">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              <div className="border border-border/50 bg-card p-3 rounded-lg flex flex-col">
                <span className="text-[10px] text-muted-foreground font-medium uppercase truncate">Temp. Hotspot</span>
                <span className="text-base font-bold mt-1">{(equipment.apiConfig?.pontoQuenteExterno || 25).toFixed(1)}°C</span>
              </div>
              <div className="border border-border/50 bg-card p-3 rounded-lg flex flex-col">
              ...
              <div className="border border-border/50 bg-card p-3 rounded-lg flex flex-col">
                ...
                <span className="text-base font-bold mt-1">{equipment.apiConfig?.tempRef || 110}°C</span>
              </div>
              <div className="border border-border/50 bg-card p-3 rounded-lg flex flex-col">
              ...
              <div className="border border-border/50 bg-card p-3 rounded-lg flex flex-col">
                ...
                <span className="text-base font-bold mt-1">{equipment.apiConfig?.p || 0}</span>
              </div>
            </div>
          </CardContent>
        </Card>
