...
const EditEquipmentModal = ({ isOpen, onClose, equipment, onSuccess }) => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const loadedFormData = {
          ...
          horas_operacao: apiConf.horas_operacao || '0',
          temp_ref: apiConf.temp_ref || '110',
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const tempEq = { api_config: formData };
    ...
    formData.temperatura_ambiente, formData.temp_ref, formData.vida_ref_anos
  ]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const apiPayload = {
        ...
        temp_ref: formData.temp_ref ? parseFloat(formData.temp_ref) : 110,
      ...
      // Check and log what changed for maintenance history
      try {
        const changedFields = [];
        ...
        if (initialFormData) {
          for (const key in formData) {
            if (formData[key] !== initialFormData[key]) {
              ...
              observacoesLinhas.push(`- ${key}: "${initialFormData[key]}" -> "${formData[key]}"`);
            }
          }
        }
        ...
        if (changedFields.length > 0) {
          ...
          await logMaintenanceHistory({
            equipment_id: equipment.id,
            tipo_manutencao: 'Modificação de Dados',
            descricao: `Atualização de informações: ${changedFields.join(', ')}`,
            ...
            <div className="space-y-6 pb-6">
              ...
              <Card className="shadow-sm border-blue-200">
                ...
                <CardContent className="pt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                  <div className="space-y-2"><Label>Horas de Operação</Label><Input type="number" step="any" value={formData.horas_operacao} onChange={e => handleChange('horas_operacao', e.target.value)} /></div>
                  <div className="space-y-2"><Label>Temp. Hotspot (°C)</Label><Input type="number" step="any" value={formData.ponto_quente_externo} onChange={e => handleChange('ponto_quente_externo', e.target.value)} /></div>
