...
const DevicesTabEquipmentList = () => {
  ...
  const getCalculatedStatus = (eq) => {
    // ✅ FIXED: Use same calculation pipeline as Dashboard and EquipmentDetails
    const healthData = calculateEquipmentHealth(eq);
    if (healthData.percentage >= 80) return 'Bom';
    if (healthData.percentage >= 50) return 'Atenção';
    return 'Crítico';
  };
  ...
  const filteredAndSortedEquipments = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      } else if (sortConfig.key === 'last_health_percentage') {
        aVal = calculateEquipmentHealth(a).percentage;
        bVal = calculateEquipmentHealth(b).percentage;
      }
                    ...
                    return (
                      <tr key={eq.id} className="bg-card hover:bg-muted/50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          {(() => {
                            const h = calculateEquipmentHealth(eq);
                            return `${h.percentage.toFixed(1)}%`;
                          })()}
                        </td>
